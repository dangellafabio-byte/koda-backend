...
def _build_conversation_system_prompt(profile: Profile, recent: List[TimelineEntry]) -> str:
    ...
    base = (
        ...
        # ============================================================
        f"REGOLE:\n"
        f"1. ⚡ LUNGHEZZA ADATTIVA — sei un VOCALE di un amico al telefono, non un saggio:\n"
        f"   • Chitchat / risposta emotiva / catarsi / 'come stai' → 1-2 frasi (max 25 parole)\n"
        f"   • Domanda concreta breve → 2-3 frasi (max 50 parole)\n"
        f"   • L'utente chiede ESPLICITAMENTE consigli, opinioni, delucidazioni, spiegazioni "
        f"     ('cosa pensi?', 'spiegami', 'consigliami', 'aiutami a capire') → 3-5 frasi "
        f"     (max 90 parole). Solo quando SERVE davvero — non dilungarti senza motivo.\n"
        f"   Default: BREVE. Se hai dubbi, fai una domanda invece di esporre tutto.\n"
