...
export async function playBridge(tier: BridgeTier = "generico"): Promise<void> {
  ...
  // === DELAY UMANO (richiesta utente 2026-06, revisione 4) ===
  // 1500-2000ms: soglia di "pazienza" prima di emettere il bridge.
  // Se la risposta vera arriva PRIMA di questo delay (caso comune per
  // domande semplici, pipeline ~1.5s) → stopBridge() cancella il timer
  // → niente intercalare, parte direttamente la risposta = comportamento
  // umano ideale ("ok ti rispondo subito senza bisogno di pause").
  // Se la pipeline supera 1.5-2s (domande complesse, ElevenLabs lento)
  // → bridge parte a coprire l'attesa.
  const delay = 1500 + Math.floor(Math.random() * 500); // 1500-2000ms
